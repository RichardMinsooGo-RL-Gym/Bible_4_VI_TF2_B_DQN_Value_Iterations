...
class NoisyAgent:  # Multi-Step TD Learning Based on Dueling Double DQN with Proportional Prioritization
    ...
    def train(self):
        ...
        for t in range(1, self.train_nums):
            action, q_values = self.model.action_value(obs[None])  # input the obs to the network model
            # action = self.get_action(best_action)   # get the real action -- no need the e-greedy exploration
...
if __name__ == '__main__':
    ...
    agent = NoisyAgent(model, target_model, env)
    # test before
