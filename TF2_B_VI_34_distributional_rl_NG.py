"""
A simple version of Distributional RL Based on Multi-Step Dueling Double DQN with Prioritized Experience Replay.
To play CartPole-v0.
...
class Model(tf.keras.Model):
    def __init__(self, num_actions, num_atoms):
        super().__init__(name='basic_distributional_rl')
        self.num_actions = num_actions
        self.num_atoms = num_atoms
        # you can try different kernel initializer
        self.shared_fc1 = kl.Dense(32, activation='relu', kernel_initializer='he_uniform')
        self.shared_fc2 = kl.Dense(32, activation='relu', kernel_initializer='he_uniform')
        # still use the dueling network architecture, but now:
        # V | v_0| v_1| ... | v_N-1|
        # A |a1_0|a1_1| ... |a1_N-1|
        #   |a2_0|a2_1| ... |a2_N-1|
        #       .        .      .
        #       .        .      .
        #   |an_0|an_1| ... |an_N-1|
        # the output of that layer is a (|A| + 1) * N dimension tensor
        # each column is a |A| + 1 dimension tensor for each atom.
        self.val_adv_fc = kl.Dense((num_actions + 1) * num_atoms, activation='relu', kernel_initializer='he_uniform')
    ...
    def call(self, inputs):
        ...
        val_adv = tf.reshape(val_adv, [-1, self.num_actions + 1, self.num_atoms])
        outputs = tf.expand_dims(val_adv[:, 0], 1) + (val_adv[:, 1:] - tf.reduce_mean(val_adv[:, 1:], -1, keepdims=True))
        # you may need tf.nn.log_softmax()
        outputs = tf.nn.softmax(outputs, axis=-1)

        return outputs

    # a* = argmax_a' Q(s, a')
    def action_value(self, obs, support_z):
        r_distribute = self.predict(obs)
        q_values = np.sum(r_distribute * support_z, axis=-1)
...
def test_model():
    num_atoms = 11
    support_z = np.linspace(-5.0, 5.0, num_atoms)
    env = gym.make('CartPole-v0')
    print('num_actions: ', env.action_space.n)
    model = Model(env.action_space.n, num_atoms)

    obs = env.reset()
    print('obs_shape: ', obs.shape)

    # tensorflow 2.0 eager mode: no feed_dict or tf.Session() needed at all
    best_action, q_values = model.action_value(obs[None], support_z)
...
class DISTAgent:  # Distributional RL Based on Multi-Step Dueling Double DQN with Proportional Prioritization
    def __init__(self, model, target_model, env, learning_rate=.001, epsilon=.1, epsilon_dacay=0.995, min_epsilon=.01,
                 gamma=.9, batch_size=8, target_update_iter=400, train_nums=5000, buffer_size=300, replay_period=20,
                 alpha=0.4, beta=0.4, beta_increment_per_sample=0.001, n_step=3, atom_num=11, vmin=-3.0, vmax=3.0):
        self.model = model
        ...
        self.vmin = vmin
        self.vmax = vmax
        self.support_z = np.expand_dims(np.linspace(vmin, vmax, atom_num), 0)
    ...
    def _kl_loss(self, y_target, y_pred):  # cross_entropy loss
        return tf.reduce_mean(self.is_weight * tf.nn.softmax_cross_entropy_with_logits(labels=y_pred, logits=y_target))

    def train(self):
        # initialize the initial observation of the agent
        obs = self.env.reset()
        for t in range(1, self.train_nums):
            best_action, _ = self.model.action_value(obs[None], self.support_z)  # input the obs to the network model
    ...
    def train_step(self):
        ...
        # Double Q-Learning
        best_action_idxes, _ = self.model.action_value(self.b_next_states, self.support_z)  # get actions through the current network
        target_distrib = self.get_target_value(self.b_next_states)   # get target distrib through the target network
        target_q = np.sum(target_distrib * self.support_z, axis=-1)
        ...
        predict_distrib  = self.model.predict(self.b_obs)
        predict_q = np.sum(predict_distrib * self.support_z, axis=-1)
        ...
        Tdistrib = target_distrib[np.arange(target_distrib.shape[0]), best_action_idxes]
        perjected_distrib = self.projected_distrib(Tdistrib)

        # perjected_distrib = np.c_[perjected_distrib, self.b_actions]
        null_distrib = np.zeros_like(predict_distrib)
        for i, val in enumerate(self.b_actions):
            null_distrib[i][val] = perjected_distrib[i]

        # print(self.b_actions)
        # print(perjected_distrib[:, -1].astype(np.int32))
        # print(predict_distrib[np.arange(self.batch_size), perjected_distrib[:, -1].astype(np.int32)])

        losses = self.model.train_on_batch(self.b_obs, null_distrib)
    ...
    def evaluation(self, env, render=True):
        ...
        while not done:
            action, q_values = self.model.action_value(obs[None], self.support_z)  # Using [None] to extend its dimension (4,) -> (1, 4)
    ...
    def projected_distrib(self, Tdistrib):
        Tz = np.broadcast_to(self.support_z, [self.batch_size, self.support_z.shape[1]])
        Tz = (self.gamma ** self.n_step) * Tz
        # stupid tensorflow don't know how to broad cast, angry!!!
        for i in range(self.batch_size):
            Tz[i] += self.b_rewards[i]
        Tz = tf.clip_by_value(Tz, self.vmin, self.vmax)
        m = np.zeros_like(Tdistrib)
        b = (Tz - self.vmin) / self.delta_z
        l, u = tf.cast(tf.math.floor(b), tf.int32), tf.cast(tf.math.ceil(b), tf.int32)
        assert m.shape == l.shape
        Ldistrib = Tdistrib * (tf.cast(u, tf.float64) - b)
        Udistrib = Tdistrib * (b - tf.cast(l, tf.float64))
        for i in range(self.batch_size):
            np.add.at(m[i], np.asarray(l)[i], Ldistrib[i])
            np.add.at(m[i], np.asarray(u)[i], Udistrib[i])
...
if __name__ == '__main__':
    ...
    model = Model(num_actions, num_atoms)
    target_model = Model(num_actions, num_atoms)
    agent = DISTAgent(model, target_model, env, atom_num=num_atoms)
    # test before
