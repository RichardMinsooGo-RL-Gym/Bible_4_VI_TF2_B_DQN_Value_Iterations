"""
A simple version of Dueling Double DQN with Prioritized Experience Replay. Just slightly modify the network architecture.
To play CartPole-v0.
...
class Model(tf.keras.Model):
    def __init__(self, num_actions):
        super().__init__(name='basic_prdddqn')
        ...
        self.logits = kl.Dense(num_actions + 1, activation='relu', kernel_initializer='he_uniform')

    # forward propagation
    def call(self, inputs):
        x = self.fc1(inputs)
        x = self.fc2(x)
        val_adv = self.logits(x)
...
class DDDQNAgent:  # Dueling Double DQN with Proportional Prioritization
    def __init__(self, model, target_model, env, learning_rate=.001, epsilon=.1, epsilon_dacay=0.995, min_epsilon=.01,
                 gamma=.9, batch_size=8, target_update_iter=400, train_nums=2000, buffer_size=300, replay_period=20,
...
if __name__ == '__main__':
    ...
    agent = DDDQNAgent(model, target_model, env)
    # test before
